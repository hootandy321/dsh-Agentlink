...
Agentlink now preflights the live Agent Preset roster, verifies the resolved preset twice before the first prompt, and preserves a content-free launch record for delegate/status diagnostics. The remaining gap is automatic selection and typed initialization for plugins that need more than preset selection. Support still means **preset-based plugins installed and configured by the user**; it does not mean arbitrary DSH plugins are automatically discovered or adapted.
