...
- A workspace claim is cooperative, persistent across turn completion, and shared only by bridge processes using the same bridge home. While a task holds `exclusive-write`, the supervising Claude Code session must not edit that cwd. DSH Web, a different bridge home, and ordinary shell/editor writes are outside enforcement.
