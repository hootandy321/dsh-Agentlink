...
- A workspace claim is cooperative, persistent across turn completion, and shared only by bridge processes using the same bridge home. It does not select, enforce, or verify the DSH Host filesystem sandbox; read `workspaceClaimSemantics.controlsDshSandbox=false` literally. While a task holds `exclusive-write`, the supervising Claude Code session must not edit that cwd. DSH Web, a different bridge home, and ordinary shell/editor writes are outside enforcement.
