...
2. Call `dsh_delegate` with a complete prompt and an existing absolute `cwd`. Omit model and normally omit `agentPreset`; the bridge's installation-time default owns that choice. The shipped `code` preset keeps the standard capability set through DSH Code Mode and is the preferred default for implementation or multi-step tool work. Override the preset only when the task needs a known installed alternative; do not choose `minimal` for writable work because its shipped filesystem composition does not use the Host filesystem sandbox. Use the default `exclusive-write` workspace claim for edits, preferably on a dedicated git worktree; use `read-only` only for work that cannot mutate files. Pass `caller` only when you know the supervising client/model exactly, for example `{ client: "codex", model: { provider: "openai", id: "gpt-6-astra", serviceTier: "standard", source: "caller-reported" } }`; if `serviceTier` is omitted for the caller comparison model, the bridge records `standard` API pricing. Otherwise the bridge records client `codex` with an unknown model. Keep the returned BridgeTask id, root session id, runId, and submission id.
...
8. Treat every approval as sandbox escalation. Never auto-allow. Use `dsh_resolve_approval(..., outcome="allow_once")` only after the supervising user/Codex approval boundary; `reject` is the fail-closed response.
...
- `dsh_status`, `dsh_wait`, `dsh_tail`, and `dsh_list` return compact supervision views by default. Request detail categories such as `result`, `interactions`, `workspace`, `sessions`, `connection`, `recovery`, or `cost` only when needed; do not ask for broad details as a routine polling pattern. `include=["cost"]` returns a small Agentlink companion summary when the DSH companion plugin is installed.
...
- A workspace claim is cooperative, persistent across turn completion, and shared only by bridge processes using the same bridge home. While a task holds `exclusive-write`, the supervising Codex must not edit that cwd. DSH Web, a different bridge home, and ordinary shell/editor writes are outside enforcement.
